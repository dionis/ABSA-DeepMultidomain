...
def build_tokenizer(fnames, max_seq_len, dat_fname):
    if os.path.exists(dat_fname):
        ...
        text = ''
        for fname in fnames:
            for key, ifile in fname.items():
                fin = open(ifile, 'r', encoding='utf-8', newline='\n', errors='ignore')
                lines = fin.readlines()
                fin.close()
...
class Tokenizer4Bert:
    def __init__(self, max_seq_len, pretrained_bert_name):
        self.tokenizer = BertTokenizer.from_pretrained(pretrained_bert_name,cache_dir="pretrained/bert/")
        self.max_seq_len = max_seq_len
    ...
    def __init__(self, fname, tokenizer):
        ...
        if type(fname) is dict :
            ...
            for domain, address in fname.items():
                fin = open(address, 'r', encoding='utf-8', newline='\n', errors='ignore')
                lines = fin.readlines()
                fin.close()
